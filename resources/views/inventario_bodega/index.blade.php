...
<div class="inventory-page">
    ...
    @elseif($vista === 'ingresos')
        <section class="inventory-workspace">
            ...
            <aside class="inventory-side-note"><i class="bi bi-shield-check"></i><strong>El historial no se edita.</strong><span>Si hubo un error, registra un ajuste o movimiento inverso para conservar la trazabilidad.</span></aside>
        </section>
        <section class="inventory-section">
            ...
            <div class="inventory-table-wrap"><table class="inventory-table"><thead><tr><th>Codigo</th><th>Recepcion</th><th>Ubicacion</th><th>Documento</th><th>Proveedor</th><th class="text-end">Lineas</th></tr></thead><tbody>@forelse($ingresos as $ingreso)<tr><td><span class="inventory-code">{{ $ingreso->codigo }}</span></td><td>{{ optional($ingreso->fecha_recepcion)->format('d/m/Y') }}</td><td>{{ $ingreso->ubicacion->nombre ?? '-' }}</td><td>{{ $ingreso->tipo_documento }} {{ $ingreso->numero_documento ?: '-' }}</td><td>{{ $ingreso->proveedor->nombre ?? 'Sin proveedor' }}</td><td class="text-end">{{ $ingreso->items->count() }}</td></tr>@empty<tr><td colspan="6" class="inventory-empty">Aun no hay ingresos registrados.</td></tr>@endforelse</tbody></table></div>
        </section>
                    ...
                    <div class="inventory-details-body">
                        <div class="inventory-split-forms">
                            ...
                            <form method="POST" action="{{ route('inventario-bodega.productos.importar') }}" enctype="multipart/form-data" class="inventory-compact-form">@csrf
                                <h3>Importar catalogo</h3><p>Admite la plantilla SAEP y la lista EPP con columnas Tipo, Categoria, Sub Categoria, Item y Formato. Los sufijos T-39, T-M y T-NA se convierten en tallas o variantes.</p><p class="inventory-form-help">Esta carga crea o actualiza productos y tallas; no modifica ni crea stock. El saldo inicial se ingresa desde un conteo fisico por ubicacion.</p><a href="{{ route('inventario-bodega.productos.plantilla') }}" class="inventory-link"><i class="bi bi-download"></i>Descargar plantilla</a><label>Archivo Excel o CSV<input name="archivo" type="file" accept=".xlsx,.xls,.csv" class="form-control" required></label><button class="btn btn-light inventory-btn" type="submit"><i class="bi bi-upload"></i>Importar productos</button>
                            </form>
                        </div>
                        ...
                        @if($canEdit && $products->isNotEmpty())
                            <form method="POST" id="product-editor" data-action-base="{{ url('inventario-bodega/productos') }}" class="inventory-editor">@csrf @method('PUT')
                                <div><h3>Editar producto existente</h3><p>El codigo se mantiene estable para no romper importaciones ni movimientos historicos.</p></div>
...
<style>
    ...
    .inventory-workspace { display:grid; grid-template-columns:minmax(0,1fr) 220px; gap:1rem; }.inventory-workspace .inventory-section { margin:0; }.inventory-side-note { align-self:start; display:grid; gap:.45rem; padding:1rem; background:#f5f3ff; border-left:3px solid #8757ec; border-radius:.55rem; color:#4a3b76; font-size:.82rem; }.inventory-side-note i { font-size:1.1rem; color:#5d37b5; }.inventory-side-note strong { color:#251352; }.inventory-side-note span { line-height:1.45; }
