...
<div class="inventory-page">
    ...
    @elseif($vista === 'kizeo')
        ...
        <section class="inventory-kizeo-notice"><i class="bi bi-shield-check"></i><div><strong>Salida centralizada</strong><span>Las entregas Kizeo siempre descuentan desde <strong>{{ $centralKizeoLocation?->nombre ?: 'SAEP-CENTRAL' }}</strong>. La aplicación masiva exige artículo y talla exactos; las tallas genéricas <strong>NA</strong> se asocian de forma segura a la variante <strong>ESTANDAR</strong> del mismo artículo. Si Kizeo se corrige luego, usa el reverso para reponer el stock y deja el motivo registrado. @if(! empty($kizeoAutoApply['enabled'])) El interruptor automático solo cubre caídas nuevas posteriores a su activación; esta cola histórica no se descuenta sola.@else El descuento automático está apagado: las caídas nuevas quedan pendientes hasta que Bodega las aplique.@endif</span></div></section>
