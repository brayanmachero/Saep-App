...
<div class="inventory-page">
    ...
    @if($vista === 'resumen')
        <section class="inventory-section inventory-filter-section">
            <form method="GET" action="{{ route('inventario-bodega.index') }}" class="inventory-filter-grid">
                ...
                <input type="hidden" name="resumen_periodo" value="{{ $summaryPeriod['period'] }}">
                @if($summaryPeriod['period'] === 'personalizado')
                    <input type="hidden" name="resumen_desde" value="{{ $summaryPeriod['requested_from']->toDateString() }}">
                    <input type="hidden" name="resumen_hasta" value="{{ $summaryPeriod['requested_to']->toDateString() }}">
                @endif
                <label>Ubicacion
        ...
        @php
            $summaryPeriodQuery = array_filter([
                'vista' => 'resumen',
                'ubicacion_id' => $selectedLocation,
                'buscar' => $summaryFilters['search'],
                'estado_stock' => $summaryFilters['stock_status'],
                'categoria' => $summaryFilters['category'],
                'subcategoria' => $summaryFilters['subcategory'],
                'proveedor_id' => $summaryFilters['provider_id'],
            ], fn ($value) => $value !== null && $value !== '');
            $summaryPeriodUrl = fn (string $period) => route('inventario-bodega.index', array_merge($summaryPeriodQuery, [
                'resumen_periodo' => $period,
                'resumen_desde' => null,
                'resumen_hasta' => null,
            ]));
        @endphp
        <section class="inventory-kizeo-period-panel inventory-summary-period-panel" aria-label="Filtro de período del resumen">
            <div class="inventory-kizeo-period-summary">
                <span class="inventory-kizeo-sync-indicator" aria-hidden="true"></span>
                <div>
                    <strong>{{ $summaryPeriod['label'] }}</strong>
                    <span>{{ number_format($summaryAnalytics['movements']) }} {{ $summaryAnalytics['movements'] === 1 ? 'movimiento operativo' : 'movimientos operativos' }} en el período</span>
                    <small>Este filtro cambia los gráficos y el kardex reciente. El saldo y el estado del catálogo siempre muestran su situación actual.</small>
                </div>
            </div>
            <form method="GET" action="{{ route('inventario-bodega.index') }}" class="inventory-kizeo-period-form inventory-summary-period-form">
                @foreach($summaryPeriodQuery as $key => $value)
                    <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                @endforeach
                <label>Periodo
                    <select name="resumen_periodo" class="form-select" data-inventory-search-ignore data-summary-period-select>
                        <option value="hoy" @selected($summaryPeriod['period'] === 'hoy')>Hoy</option>
                        <option value="ayer" @selected($summaryPeriod['period'] === 'ayer')>Ayer</option>
                        <option value="semana" @selected($summaryPeriod['period'] === 'semana')>Esta semana</option>
                        <option value="mes_actual" @selected($summaryPeriod['period'] === 'mes_actual')>Este mes</option>
                        <option value="mes_anterior" @selected($summaryPeriod['period'] === 'mes_anterior')>Mes anterior</option>
                        <option value="personalizado" @selected($summaryPeriod['period'] === 'personalizado')>Personalizado</option>
                        <option value="inicio" @selected($summaryPeriod['period'] === 'inicio')>Desde el inicio</option>
                    </select>
                </label>
                <div class="inventory-kizeo-custom-range {{ $summaryPeriod['period'] === 'personalizado' ? 'is-visible' : '' }}" data-summary-custom-range>
                    <label>Desde<input type="date" name="resumen_desde" class="form-control" value="{{ $summaryPeriod['period'] === 'personalizado' ? $summaryPeriod['requested_from']->toDateString() : '' }}"></label>
                    <label>Hasta<input type="date" name="resumen_hasta" class="form-control" value="{{ $summaryPeriod['period'] === 'personalizado' ? $summaryPeriod['requested_to']->toDateString() : '' }}"></label>
                </div>
                <button class="btn btn-primary inventory-btn" type="submit"><i class="bi bi-funnel"></i>Aplicar</button>
            </form>
            <div class="inventory-kizeo-quick-periods" aria-label="Accesos rápidos del resumen">
                @foreach(['hoy' => 'Hoy', 'ayer' => 'Ayer', 'semana' => 'Semana', 'mes_actual' => 'Mes'] as $period => $label)
                    <a href="{{ $summaryPeriodUrl($period) }}" class="{{ $summaryPeriod['period'] === $period ? 'active' : '' }}">{{ $label }}</a>
                @endforeach
            </div>
        </section>
        ...
        <section class="inventory-traceability" aria-labelledby="inventory-traceability-title">
            <div class="inventory-traceability-banner">
                <div>
                    ...
                    <h2 id="inventory-traceability-title">Entradas y salidas · {{ $summaryPeriod['label'] }}</h2>
                    <p>El saldo mantiene todo el kardex. Esta lectura usa movimientos desde {{ $summaryPeriod['tracking_start']->format('d/m/Y') }} para no mezclar cargas históricas previas.@if($summaryAnalytics['movements']) Se registran {{ number_format($summaryAnalytics['movements']) }} movimientos en {{ $summaryAnalytics['active_days'] }} {{ $summaryAnalytics['active_days'] === 1 ? 'día activo' : 'días activos' }}.@endif</p>
                </div>
                <span class="inventory-traceability-period">Período<br><strong>{{ $summaryPeriod['requested_from']->format('d/m/Y') }}@if(! $summaryPeriod['requested_from']->isSameDay($summaryPeriod['requested_to'])) — {{ $summaryPeriod['requested_to']->format('d/m/Y') }}@endif</strong></span>
            </div>
            ...
            <div class="inventory-analytics-grid">
                <article class="inventory-analytics-card inventory-analytics-card-main">
                    <div class="inventory-analytics-card-heading">
                        ...
                        <span><i class="bi bi-calendar3"></i>{{ $summaryPeriod['requested_from']->format('d/m') }} — {{ $summaryPeriod['requested_to']->format('d/m') }}</span>
                    </div>
                    @if($summaryAnalytics['movements'])
                    ...
                    @else
                        <div class="inventory-analytics-empty"><i class="bi bi-calendar-x"></i><strong>Sin movimientos operativos en este período.</strong><span>{{ $summaryPeriod['has_data_range'] ? 'Aún no hay movimientos registrados para las fechas seleccionadas.' : 'La trazabilidad comienza el '.$summaryPeriod['tracking_start']->format('d/m/Y').'.' }}</span></div>
                    @endif
                </article>
                <article class="inventory-analytics-card">
                    ...
                    @else
                        <div class="inventory-analytics-empty"><i class="bi bi-bar-chart-line"></i><strong>Sin operaciones para clasificar.</strong><span>El desglose estará disponible cuando Bodega registre movimientos.</span></div>
                    @endif
                </article>
            </div>
        </section>
                ...
                <table class="inventory-table">
                    ...
                    @forelse($balances as $variant)
                        @php
                            ...
                            $stockStatus = $isOutOfStock ? 'Sin stock' : ($isCriticalStock ? 'Crítico' : ($needsReplenishment ? 'Reponer' : 'Disponible'));
                            $stockStatusClass = $isOutOfStock ? 'is-out-of-stock' : ($isCriticalStock ? 'is-stock-critical' : ($needsReplenishment ? 'is-critical' : 'is-ok'));
                        @endphp
                        <tr class="inventory-stock-row" tabindex="0" role="button" aria-label="Ver detalle de {{ $variant->producto->nombre }}, talla {{ $variant->talla }}" data-stock-table-row data-inventory-stock-detail-url="{{ route('inventario-bodega.stock.detalle', ['variante' => $variant, 'ubicacion_id' => $selectedLocation]) }}">
                            ...
                            <td><span class="inventory-status {{ $stockStatusClass }}">{{ $stockStatus }}</span></td>
                        </tr>
                    @empty
    ...
    @elseif($vista === 'ingresos')
        <section class="inventory-section inventory-receipt-section" id="registrar-ingreso">
            <div class="inventory-section-title"><div><h2>Registrar ingreso</h2><p>Registra una compra, reposición o devolución. Las devoluciones aumentan el stock y quedan imputadas al centro de costo que las devuelve.</p></div></div>
                ...
                @else
                    <form method="POST" action="{{ route('inventario-bodega.ingresos.store') }}" class="inventory-form">
                        ...
                        <div class="inventory-form-grid three">
                            ...
                            <label>Tipo de ingreso <select name="tipo_ingreso" class="form-select" data-receipt-type required>@foreach(\App\Models\InventarioIngreso::TIPOS_INGRESO as $key => $label)<option value="{{ $key }}" @selected(old('tipo_ingreso', 'COMPRA') === $key)>{{ $label }}</option>@endforeach</select></label>
                            @if($inventoryCostCenters->isNotEmpty())<label data-receipt-cost-center hidden>Centro de costo de la devolución<select name="centro_costo_id" class="form-select" data-inventory-search-select data-search-placeholder="Buscar centro de costo"><option value="">Selecciona el centro de costo</option>@foreach($inventoryCostCenters as $costCenter)<option value="{{ $costCenter->id }}" @selected((int) old('centro_costo_id') === $costCenter->id)>{{ $costCenter->nombre }}{{ $costCenter->comuna ? ' · ' . $costCenter->comuna : '' }}</option>@endforeach</select><small>Obligatorio solo para devoluciones.</small></label>@else<label data-receipt-cost-center-unavailable hidden class="inventory-field-hint"><strong>No hay centros de costo activos.</strong> Crea uno en Maestros antes de registrar una devolución.</label>@endif
                        ...
                        <p class="inventory-form-context" data-receipt-context>Estás registrando una compra o reposición de Bodega.</p>
                        <label class="inventory-wide-label">Observacion <input name="observacion" class="form-control" maxlength="500" placeholder="Compra, reposicion u otra referencia" data-receipt-observation></label>
        ...
        <section class="inventory-section" id="historial-ingresos">
            <div class="inventory-section-title"><div><h2>Historial de ingresos</h2><p>Consulta todas las compras y recepciones registradas. Puedes buscar por artículo, proveedor, documento o persona, y acotar por fecha.</p></div></div>
            <form method="GET" action="{{ route('inventario-bodega.index') }}" class="inventory-history-filter">
                <input type="hidden" name="vista" value="ingresos">
                <label class="inventory-history-search">Buscar<input name="ingreso_buscar" value="{{ $receiptHistoryFilters['search'] }}" class="form-control" placeholder="Artículo, proveedor, documento o persona"></label>
                <label>Desde<input type="date" name="ingreso_desde" value="{{ $receiptHistoryFilters['from'] }}" class="form-control"></label>
                <label>Hasta<input type="date" name="ingreso_hasta" value="{{ $receiptHistoryFilters['to'] }}" class="form-control"></label>
                <label>Estado<select name="ingreso_estado" class="form-select"><option value="">Todos</option><option value="vigentes" @selected($receiptHistoryFilters['status'] === 'vigentes')>Vigentes</option><option value="anulados" @selected($receiptHistoryFilters['status'] === 'anulados')>Anulados</option></select></label>
                <label>Ubicación<select name="ingreso_ubicacion_id" class="form-select"><option value="">Todas</option>@foreach($locations as $location)<option value="{{ $location->id }}" @selected($receiptHistoryFilters['location_id'] === $location->id)>{{ $location->nombre }}</option>@endforeach</select></label>
                <div class="inventory-history-filter-actions"><button class="btn btn-primary inventory-btn" type="submit"><i class="bi bi-funnel"></i>Aplicar</button>@if($receiptHistoryFilters['search'] !== '' || $receiptHistoryFilters['from'] || $receiptHistoryFilters['to'] || $receiptHistoryFilters['status'] !== '' || $receiptHistoryFilters['location_id'])<a href="{{ route('inventario-bodega.index', ['vista' => 'ingresos']) }}" class="btn btn-light inventory-btn">Limpiar</a>@endif</div>
            </form>
            <div class="inventory-history-summary">@if($ingresos->total()) Mostrando {{ number_format($ingresos->firstItem()) }} a {{ number_format($ingresos->lastItem()) }} de {{ number_format($ingresos->total()) }} ingresos.@else No se encontraron ingresos con los filtros seleccionados.@endif</div>
            <div class="inventory-table-wrap"><table class="inventory-table"><thead><tr><th>Codigo</th><th>Recepcion</th><th>Ubicacion</th><th>Documento</th><th>Proveedor</th><th>Tipo / centro de costo</th><th>Estado</th><th class="text-end">Lineas</th><th>Acciones</th></tr></thead><tbody>@forelse($ingresos as $ingreso)<tr><td><span class="inventory-code">{{ $ingreso->codigo }}</span></td><td>{{ optional($ingreso->fecha_recepcion)->format('d/m/Y') }}</td><td>{{ $ingreso->ubicacion->nombre ?? '-' }}</td><td>{{ $ingreso->tipo_documento }} {{ $ingreso->numero_documento ?: '-' }}</td><td>{{ $ingreso->proveedor->nombre ?? 'Sin proveedor' }}</td><td><strong>{{ \App\Models\InventarioIngreso::TIPOS_INGRESO[$ingreso->tipo_ingreso] ?? 'Compra / reposición' }}</strong><small>{{ $ingreso->centroCosto?->nombre ?: ($ingreso->centro_costo ?: 'Sin centro de costo') }}</small></td><td>@if($ingreso->reversado_en)<span class="inventory-status is-empty">Anulado</span><small>Anulado {{ $ingreso->reversado_en->format('d/m/Y H:i') }}{{ $ingreso->reversadoPor ? ' por ' . $ingreso->reversadoPor->name : '' }}<br>{{ $ingreso->motivo_reversion }}</small>@else<span class="inventory-status is-ok">Vigente</span>@endif</td><td class="text-end">{{ $ingreso->items->count() }}</td><td><div class="inventory-table-actions"><button type="button" class="btn btn-light inventory-btn" data-inventory-detail-trigger="inventory-detail-receipt-{{ $ingreso->id }}"><i class="bi bi-eye"></i>Ver detalle</button>@if($canEdit && ! $ingreso->reversado_en)<details class="inventory-receipt-reverse"><summary class="btn btn-light inventory-btn inventory-receipt-reverse-trigger" title="Anular ingreso"><i class="bi bi-arrow-counterclockwise"></i>Anular ingreso</summary><form method="POST" action="{{ route('inventario-bodega.ingresos.revertir', $ingreso) }}" class="inventory-reverse-form">@csrf<label>Motivo de anulación<input name="motivo_reversion" class="form-control" minlength="5" maxlength="500" required placeholder="Ej.: ingreso de prueba"></label><label class="inventory-checkbox inventory-confirm-reverse"><input type="checkbox" required><span>Confirmo que se descontará el stock de estas líneas.</span></label><button type="submit" class="btn btn-light inventory-btn text-danger"><i class="bi bi-arrow-counterclockwise"></i>Confirmar anulación</button></form></details>@endif</div></td></tr>@empty<tr><td colspan="9" class="inventory-empty">Aun no hay ingresos registrados.</td></tr>@endforelse</tbody></table></div>
            <div class="inventory-pagination">{{ $ingresos->onEachSide(1)->links() }}</div>
            @foreach($ingresos as $ingreso)
                ...
                <div id="inventory-detail-receipt-{{ $ingreso->id }}" hidden data-inventory-detail-template>
                    <div class="inventory-detail-heading"><div><span class="inventory-detail-kicker">{{ $ingreso->tipo_ingreso === 'DEVOLUCION_EPP' ? 'Devolución de bodega' : 'Ingreso de bodega' }}</span><h2>{{ $ingreso->codigo }}</h2><p>Recepción {{ optional($ingreso->fecha_recepcion)->format('d/m/Y') }}</p></div><span class="inventory-status {{ $ingreso->reversado_en ? 'is-empty' : 'is-ok' }}">{{ $ingreso->reversado_en ? 'Anulado' : 'Vigente' }}</span></div>
                    <div class="inventory-detail-grid"><div><span>Ubicación</span><strong>{{ $ingreso->ubicacion->nombre ?? '-' }}</strong></div><div><span>Tipo de ingreso</span><strong>{{ \App\Models\InventarioIngreso::TIPOS_INGRESO[$ingreso->tipo_ingreso] ?? 'Compra / reposición' }}</strong></div><div><span>Centro de costo</span><strong>{{ $ingreso->centroCosto?->nombre ?: ($ingreso->centro_costo ?: 'No aplica') }}</strong></div><div><span>Proveedor</span><strong>{{ $ingreso->proveedor->nombre ?? 'Sin proveedor' }}</strong></div><div><span>Documento</span><strong>{{ $ingreso->tipo_documento }} {{ $ingreso->numero_documento ?: 'sin número' }}</strong></div><div><span>Fecha documento</span><strong>{{ optional($ingreso->fecha_documento)->format('d/m/Y') ?: 'No informada' }}</strong></div><div><span>Registrado por</span><strong>{{ $ingreso->registradoPor->nombre_completo ?? $ingreso->registradoPor->name ?? 'No disponible' }}</strong></div><div><span>Líneas</span><strong>{{ $ingreso->items->count() }} artículo(s)</strong></div></div>
    ...
    @elseif($vista === 'movimientos')
        ...
        <section class="inventory-section" id="historial-movimientos">
            <div class="inventory-section-title"><div><h2>Kardex histórico</h2><p>Consulta todos los movimientos registrados: ingresos, entregas, traslados, ajustes y reversos.</p></div></div>
            <form method="GET" action="{{ route('inventario-bodega.index') }}" class="inventory-history-filter">
                <input type="hidden" name="vista" value="movimientos">
                <label class="inventory-history-search">Buscar<input name="movimiento_buscar" value="{{ $movementHistoryFilters['search'] }}" class="form-control" placeholder="Artículo, destinatario, documento o persona"></label>
                <label>Desde<input type="date" name="movimiento_desde" value="{{ $movementHistoryFilters['from'] }}" class="form-control"></label>
                <label>Hasta<input type="date" name="movimiento_hasta" value="{{ $movementHistoryFilters['to'] }}" class="form-control"></label>
                <label>Tipo<select name="movimiento_tipo" class="form-select"><option value="">Todos</option>@foreach(\App\Models\InventarioMovimiento::TIPOS as $key => $label)<option value="{{ $key }}" @selected($movementHistoryFilters['type'] === $key)>{{ $label }}</option>@endforeach</select></label>
                <label>Ubicación<select name="movimiento_ubicacion_id" class="form-select"><option value="">Todas</option>@foreach($locations as $location)<option value="{{ $location->id }}" @selected($movementHistoryFilters['location_id'] === $location->id)>{{ $location->nombre }}</option>@endforeach</select></label>
                <div class="inventory-history-filter-actions"><button class="btn btn-primary inventory-btn" type="submit"><i class="bi bi-funnel"></i>Aplicar</button>@if($movementHistoryFilters['search'] !== '' || $movementHistoryFilters['from'] || $movementHistoryFilters['to'] || $movementHistoryFilters['type'] !== '' || $movementHistoryFilters['location_id'])<a href="{{ route('inventario-bodega.index', ['vista' => 'movimientos']) }}" class="btn btn-light inventory-btn">Limpiar</a>@endif</div>
            </form>
            <div class="inventory-history-summary">@if($movements->total()) Mostrando {{ number_format($movements->firstItem()) }} a {{ number_format($movements->lastItem()) }} de {{ number_format($movements->total()) }} movimientos.@else No se encontraron movimientos con los filtros seleccionados.@endif</div>
            ...
            <div class="inventory-pagination">{{ $movements->onEachSide(1)->links() }}</div>
    ...
    @elseif($vista === 'conteos')
        ...
        <section class="inventory-section">
            <div class="inventory-section-title"><div><h2>Conteos registrados</h2><p>Los borradores y los que están en revisión se pueden eliminar; los aprobados ya dejaron su ajuste en el kardex y no se borran.</p></div></div>
            <div class="inventory-table-wrap"><table class="inventory-table"><thead><tr><th>Codigo</th><th>Ubicacion</th><th>Fecha corte</th><th>Estado</th><th>Lineas</th><th>Accion</th></tr></thead><tbody>@forelse($conteos as $conteo)<tr><td><span class="inventory-code">{{ $conteo->codigo }}</span></td><td>{{ $conteo->ubicacion->nombre ?? '-' }}</td><td>{{ optional($conteo->fecha_corte)->format('d/m/Y') }}</td><td><span class="inventory-status {{ $conteo->estado === 'APROBADO' ? 'is-ok' : ($conteo->estado === 'EN_REVISION' ? 'is-review' : 'is-empty') }}">{{ \App\Models\InventarioConteo::ESTADOS[$conteo->estado] ?? str_replace('_', ' ', $conteo->estado) }}</span></td><td>{{ $conteo->lineas_count ?? $conteo->lineas()->count() }}</td><td><div class="inventory-table-actions"><a href="{{ route('inventario-bodega.conteos.show', $conteo) }}" class="btn btn-light inventory-icon-btn" title="Abrir conteo"><i class="bi bi-arrow-up-right-square"></i></a>@if($canEdit && $conteo->puedeEliminarse())<form method="POST" action="{{ route('inventario-bodega.conteos.destroy', $conteo) }}" onsubmit="return confirm('Se eliminará el conteo {{ $conteo->codigo }} y sus líneas. No toca el kardex. ¿Continuar?');">@csrf @method('DELETE')<button type="submit" class="btn btn-light inventory-icon-btn text-danger" title="Eliminar conteo de prueba"><i class="bi bi-trash3"></i></button></form>@endif</div></td></tr>@empty<tr><td colspan="6" class="inventory-empty">Aun no hay conteos registrados.</td></tr>@endforelse</tbody></table></div>
        </section>
    ...
    @elseif($vista === 'kizeo')
        <section class="inventory-kizeo-intro">
            ...
            <div class="inventory-kizeo-intro-actions">
                @if($canEdit)
                    @php
                        $autoApplyEnabled = (bool) ($kizeoAutoApply['enabled'] ?? false);
                        $autoApplySince = $kizeoAutoApply['since'] ?? null;
                        $autoApplyConfirm = $autoApplyEnabled
                            ? 'Las nuevas entregas de Kizeo volverán a quedar pendientes para aplicarlas a mano. ¿Desactivar el descuento automático?'
                            : 'Las entregas ya sincronizadas NO se descontarán. Solo las nuevas que lleguen a Kizeo después de activar, con artículo y talla reconocidos y stock en Sede Central. ¿Activar el descuento automático?';
                    @endphp
                    <form method="POST" action="{{ route('inventario-bodega.entregas-kizeo.auto-aplicar') }}" class="inventory-kizeo-auto-form">
                        @csrf
                        <input type="hidden" name="activo" value="{{ $autoApplyEnabled ? 0 : 1 }}">
                        <button type="submit" class="inventory-kizeo-switch {{ $autoApplyEnabled ? 'is-on' : '' }}" onclick="return confirm(@js($autoApplyConfirm))">
                            <span class="inventory-kizeo-switch-track" aria-hidden="true"><span class="inventory-kizeo-switch-thumb"></span></span>
                            <span>
                                <strong>{{ $autoApplyEnabled ? 'Descuento automático activo' : 'Descuento automático desactivado' }}</strong>
                                @if($autoApplyEnabled && $autoApplySince)
                                    <small>Nuevas desde {{ $autoApplySince->timezone(config('app.timezone'))->format('d/m/Y H:i') }}</small>
                                @else
                                    <small>Actívalo cuando el stock de Central esté al día. No aplica el histórico.</small>
                                @endif
                            </span>
                        </button>
                    </form>
        ...
        @php
            ...
            $kizeoDeliveryCount = method_exists($kizeoDeliveries, 'total') ? $kizeoDeliveries->total() : $kizeoDeliveries->count();
        @endphp
        <section class="inventory-kizeo-period-panel" aria-label="Filtro de entregas Kizeo">
            <div class="inventory-kizeo-period-summary">
                <span class="inventory-kizeo-sync-indicator" aria-hidden="true"></span>
                <div>
                    <strong>{{ $kizeoPeriod['label'] }}</strong>
                    <span>{{ number_format($kizeoDeliveryCount) }} {{ $kizeoDeliveryCount === 1 ? 'entrega sincronizada' : 'entregas sincronizadas' }} en SAEP</span>
                    @if($kizeoLastSyncedAt)
                        <small>Última sincronización: {{ $kizeoLastSyncedAt->timezone(config('app.timezone'))->format('d/m/Y H:i') }}</small>
                    @endif
                </div>
            </div>
            <form method="GET" action="{{ route('inventario-bodega.index') }}" class="inventory-kizeo-period-form">
                <input type="hidden" name="vista" value="kizeo">
                <input type="hidden" name="ubicacion_id" value="{{ $selectedLocation }}">
                <input type="hidden" name="kizeo_origen" value="{{ $kizeoQueue }}">
                <label>Periodo
                    <select name="kizeo_periodo" class="form-select" data-inventory-search-ignore data-kizeo-period-select>
                        <option value="hoy" @selected($kizeoPeriod['period'] === 'hoy')>Hoy</option>
                        <option value="ayer" @selected($kizeoPeriod['period'] === 'ayer')>Ayer</option>
                        <option value="antes_ayer" @selected($kizeoPeriod['period'] === 'antes_ayer')>Antes de ayer</option>
                        <option value="semana" @selected($kizeoPeriod['period'] === 'semana')>Esta semana</option>
                        <option value="mes_actual" @selected($kizeoPeriod['period'] === 'mes_actual')>Este mes</option>
                        <option value="mes_anterior" @selected($kizeoPeriod['period'] === 'mes_anterior')>Mes anterior</option>
                ...
                <div class="inventory-kizeo-custom-range {{ $kizeoPeriod['period'] === 'personalizado' ? 'is-visible' : '' }}" data-kizeo-custom-range>
                    ...
                    <label>Hasta<input type="date" name="kizeo_hasta" class="form-control" value="{{ $kizeoPeriod['period'] === 'personalizado' ? $kizeoPeriod['to']?->toDateString() : '' }}"></label>
                </div>
                <button class="btn btn-primary inventory-btn" type="submit"><i class="bi bi-funnel"></i>Aplicar</button>
            </form>
            <div class="inventory-kizeo-quick-periods" aria-label="Accesos rápidos por período">
                @foreach(['hoy' => 'Hoy', 'ayer' => 'Ayer', 'semana' => 'Semana', 'mes_actual' => 'Mes'] as $period => $label)
                    <a href="{{ $kizeoPeriodUrl($period) }}" class="{{ $kizeoPeriod['period'] === $period ? 'active' : '' }}">{{ $label }}</a>
                @endforeach
            </div>
        </section>
        ...
        @elseif($canCreate && ($kizeoQueue ?? 'vigentes') !== 'historico')
        ...
        <section class="inventory-kizeo-queue">
                    ...
                    <summary class="inventory-delivery-header" title="Abrir o cerrar el detalle de esta entrega">
                        ...
                        <div class="inventory-delivery-status">
                            ...
                            @elseif($isHistorical && ! $application)
                                <span class="inventory-status is-empty" title="Formulario histórico de Kizeo. No se descuenta del inventario actual.">Histórico · no descuenta</span>
                    ...
                    <div class="inventory-delivery-body">
                        ...
                        @elseif(! $application)
                            ...
                            @else
                                <form method="POST" action="{{ route('inventario-bodega.entregas-kizeo.aplicar', $delivery) }}" class="inventory-kizeo-form">
                                    ...
                                    <div class="inventory-table-wrap"><table class="inventory-table inventory-table-compact"><thead><tr><th>Articulo informado</th><th>Talla</th><th class="text-end">Cantidad</th><th>Relacion con inventario</th><th>Stock SAEP</th></tr></thead><tbody>
                                        @foreach($deliveryItems as $item)
                                            @php
                                                $selectedVariantId = $deliverySuggestions[$item->id] ?? null;
                                                $selectedVariant = $selectedVariantId === null ? null : $variantOptions->find($selectedVariantId);
                                                $requestedQuantity = (float) $item->cantidad;
                                                $availableStock = $selectedVariantId === null ? null : (float) ($kizeoCentralStockByVariant[$selectedVariantId] ?? 0);
                                                $formattedStock = $availableStock === null ? '—' : rtrim(rtrim(number_format($availableStock, 3, ',', '.'), '0'), ',');
                                                $stockAdjustmentUrl = $selectedVariant && $availableStock !== null && $availableStock < $requestedQuantity && $canEdit
                                                    ? route('inventario-bodega.index', [
                                                        'vista' => 'catalogo',
                                                        'ubicacion_id' => $centralKizeoLocation->id,
                                                        'producto_editar' => $selectedVariant->producto_id,
                                                        'producto_buscar' => $selectedVariant->producto->codigo,
                                                        'variante_ajustar' => $selectedVariant->id,
                                                    ])
                                                    : null;
                                            @endphp
                                            <tr><td><strong>{{ $item->articulo ?: 'Sin articulo' }}</strong></td><td>{{ $item->talla ?: 'Estandar' }}</td><td class="text-end">{{ rtrim(rtrim(number_format($requestedQuantity, 3, ',', '.'), '0'), ',') }}</td><td><select name="lineas[{{ $item->id }}][variante_id]" class="form-select form-select-sm" required data-inventory-search-select data-kizeo-stock-select data-search-placeholder="Buscar por codigo, articulo o talla"><option value="">Selecciona articulo y talla</option>@foreach($variantOptions as $variant)<option value="{{ $variant->id }}" @selected($selectedVariantId === $variant->id)>{{ $variant->producto->codigo }} - {{ $variant->producto->nombre }} · {{ $variant->talla }}</option>@endforeach</select></td><td><output class="inventory-kizeo-stock{{ $availableStock === null ? ' is-unlinked' : ($availableStock < $requestedQuantity ? ' is-low' : ' is-ok') }}" data-kizeo-stock data-requested-quantity="{{ $requestedQuantity }}"><span>{{ $availableStock === null ? 'Sin relación' : ($availableStock < $requestedQuantity ? 'Insuficiente' : 'Disponible') }}</span><strong>{{ $formattedStock }}</strong></output>@if($stockAdjustmentUrl)<a href="{{ $stockAdjustmentUrl }}" class="inventory-kizeo-adjust-link" title="Registrar el saldo físico de esta talla en Sede Central SAEP"><i class="bi bi-pencil-square"></i>Ajustar stock</a>@endif</td></tr>
                                        @endforeach
                                    </tbody></table></div>
                        ...
                        @else
                            ...
                            <div class="inventory-kizeo-application-lines">
                                    ...
                                    <div class="inventory-table-wrap inventory-kizeo-applied-lines-table"><table class="inventory-table inventory-table-compact"><thead><tr><th>{{ $isReturn ? 'Artículo informado al devolver' : 'Artículo informado al descontar' }}</th><th>{{ $isReturn ? 'Artículo ingresado en SAEP' : 'Artículo descontado en SAEP' }}</th><th>Talla</th><th class="text-end">Cantidad</th></tr></thead><tbody>
                                        @foreach($application->lineas as $line)
                                            @php
                                                ...
                                                $discountedProduct = $discountedVariant?->producto;
                                                $discountedQuantity = (float) $line->cantidad_fuente;
                                            @endphp
                                            <tr>
                                ...
                                @if($correctionMovements->isNotEmpty())
                                    <section class="inventory-kizeo-line-panel is-correction">
                                        <div class="inventory-kizeo-line-panel-heading">
                                            <div>
                                        ...
                                        <div class="inventory-table-wrap inventory-kizeo-correction-table"><table class="inventory-table inventory-table-compact"><thead><tr><th>Acción aplicada</th><th>Artículo</th><th>Talla</th><th class="text-end">Cambio de stock</th><th>Motivo</th></tr></thead><tbody>
                                            @foreach($correctionMovements as $movement)
                                                @php
                                                    $wasRestocked = (float) $movement->cantidad > 0;
                                                    $correctionProduct = $movement->producto ?: $movement->variante?->producto;
                                                    $correctionDetail = preg_replace('/^Corrección automática Kizeo [^·]+ ·\s*/u', '', (string) $movement->observacion) ?: 'Cambio informado desde Kizeo.';
                                                    $correctionQuantity = rtrim(rtrim(number_format(abs((float) $movement->cantidad), 3, ',', '.'), '0'), ',');
                                                @endphp
                                                <tr>
                                                    <td><span class="inventory-kizeo-correction-action {{ $wasRestocked ? 'is-restocked' : 'is-discounted' }}"><i class="bi {{ $wasRestocked ? 'bi-plus-circle' : 'bi-dash-circle' }}"></i>{{ $wasRestocked ? 'Stock repuesto' : 'Stock descontado' }}</span></td>
                                                    <td>@if($correctionProduct)<strong>{{ $correctionProduct->nombre }}</strong><small>{{ $correctionProduct->codigo }}</small>@else <span class="inventory-muted">Artículo no disponible</span> @endif</td>
                                                    <td>{{ $movement->variante?->talla ?: '—' }}</td>
                                                    <td class="text-end"><strong class="{{ $wasRestocked ? 'inventory-kizeo-restocked-quantity' : 'inventory-kizeo-discounted-quantity' }}">{{ $wasRestocked ? '+' : '−' }}{{ $correctionQuantity }}</strong></td>
                                                    <td><span class="inventory-kizeo-correction-detail">{{ ucfirst($correctionDetail) }}</span></td>
                                                </tr>
                                            @endforeach
                                        </tbody></table></div>
                                    </section>
                                ...
                                @if($showCurrentSource)
                                    <section class="inventory-kizeo-line-panel is-current-source">
                                        ...
                                        <div class="inventory-table-wrap inventory-kizeo-current-lines-table"><table class="inventory-table inventory-table-compact"><thead><tr><th>Artículo informado actualmente</th><th>Talla</th><th class="text-end">Cantidad</th></tr></thead><tbody>
                                            @forelse($deliveryItems as $item)
                                                <tr><td><strong>{{ $item->articulo ?: 'Sin artículo informado' }}</strong><small>Línea {{ $item->linea }}</small></td><td>{{ $item->talla ?: '—' }}</td><td class="text-end">{{ rtrim(rtrim(number_format((float) $item->cantidad, 3, ',', '.'), '0'), ',') }}</td></tr>
                                            @empty
                                                <tr><td colspan="3" class="inventory-muted">Kizeo ya no informa líneas válidas para este comprobante.</td></tr>
                                            @endforelse
                                        </tbody></table></div>
                                    </section>
                                @endif
            ...
            @empty
                <section class="inventory-section"><div class="inventory-empty">No hay entregas sincronizadas para <strong>{{ strtolower($kizeoPeriod['label']) }}</strong>. Cambia el período o sincroniza Kizeo desde Entregas EPP para ver nuevos comprobantes.</div></section>
            @endforelse
        </section>
    ...
    @else
        <section class="inventory-section" id="catalogo">
            <div class="inventory-section-title"><div><h2>Catalogo autogestionable</h2><p>Productos, ubicaciones y proveedores se administran aqui. Inhabilitar un producto no borra su historial: deja de usarse y se puede reactivar después. Al sincronizar, Kizeo deja de ofrecerlo en entregas nuevas.</p></div></div>
                ...
                <details class="inventory-details" open>
                    ...
                    <div class="inventory-details-body">
                        <section class="inventory-catalog-sync">
                            <div><strong><i class="bi bi-cloud-arrow-up"></i> SAEP publica el catálogo en Kizeo</strong><span>La lista avanzada <strong>EPP AVANZADA ({{ $kizeoCatalogListId ?: 'sin configurar' }})</strong> recibe nombre, talla, tipo, categoría, subcategoría y formato. Los productos inactivos se quitan de las opciones futuras. Stock, mínimos, costos e historial se mantienen exclusivamente en SAEP.</span></div>
                            @if($canEdit && $kizeoCatalogListId)
                                <form method="POST" action="{{ route('inventario-bodega.catalogo.kizeo.sincronizar') }}">@csrf<button class="btn btn-light inventory-btn" type="submit" onclick="return confirm('Se publicarán en Kizeo los cambios del catálogo SAEP y se quitarán los productos inhabilitados de las opciones futuras. El historial de SAEP no se borra. ¿Continuar?')"><i class="bi bi-arrow-repeat"></i>Sincronizar ahora</button></form>
                            @endif
                        </section>
                        <div class="inventory-split-forms">
                            ...
                            <form method="POST" action="{{ route('inventario-bodega.productos.importar') }}" enctype="multipart/form-data" class="inventory-compact-form">@csrf
                                <h3>Importar catalogo</h3><p>Admite la plantilla SAEP y la lista EPP con columnas Tipo, Categoria, Sub Categoria, Item y Formato. Los sufijos T-39, T-M y T-NA se convierten en tallas o variantes.</p><div class="inventory-import-flow"><i class="bi bi-box-seam"></i><div><strong>El catálogo puede incluir stock, costo y estado por talla.</strong><span>Completa Costo_Referencia por talla para conservar el valor vigente y su evolución. Estado acepta Activo/Habilitado o Inactivo/Inhabilitado; si dejas Codigo vacío, SAEP lo genera. Cero o vacío en costo no reemplaza un valor ya registrado.</span></div></div><div class="inventory-import-actions"><a href="{{ route('inventario-bodega.index', ['vista' => 'ingresos']) }}" class="inventory-link"><i class="bi bi-box-arrow-in-down"></i>Cargar desde compra</a><a href="{{ route('inventario-bodega.index', ['vista' => 'conteos']) }}" class="inventory-link"><i class="bi bi-clipboard-check"></i>Cargar desde conteo</a></div><a href="{{ route('inventario-bodega.productos.plantilla') }}" class="inventory-template-download"><i class="bi bi-download"></i>Descargar plantilla</a><label>Archivo Excel o CSV<input name="archivo" type="file" accept=".xlsx,.xls,.csv" class="form-control" required></label><button class="btn btn-light inventory-btn" type="submit"><i class="bi bi-upload"></i>Importar productos</button>
                            </form>
                        </div>
                        <form method="GET" action="{{ route('inventario-bodega.index') }}" class="inventory-product-search">
                            <input type="hidden" name="vista" value="catalogo">
                            <label>Buscar<input class="form-control" name="producto_buscar" value="{{ $productSearch }}" placeholder="Buscar por codigo, nombre o categoria"></label>
                            <label>Estado
                                <select name="producto_estado" class="form-select">
                                    <option value="">Todos</option>
                                    <option value="activos" @selected($productStatus === 'activos')>Activos</option>
                                    <option value="inactivos" @selected($productStatus === 'inactivos')>Inactivos</option>
                                </select>
                            </label>
                            <button class="btn btn-light inventory-btn" type="submit"><i class="bi bi-search"></i>Buscar</button>
                            @if($productSearch !== '' || $productStatus !== '')<a href="{{ route('inventario-bodega.index', ['vista' => 'catalogo']) }}" class="btn btn-light inventory-icon-btn" title="Limpiar filtros"><i class="bi bi-x-lg"></i></a>@endif
                        </form>
                        ...
                        @if($canEdit && $productEditorOptions->isNotEmpty())
                            ...
                            @if($activeLocations->isNotEmpty())
                                <section id="product-variant-editor" class="inventory-variant-editor" data-action="{{ route('inventario-bodega.stock-talla.store') }}" data-variant-status-action-base="{{ url('inventario-bodega/variantes') }}" data-csrf="{{ csrf_token() }}" data-stocks="{{ $variantStocksByLocation->toJson() }}" data-product-page="{{ $products->currentPage() }}" data-product-search="{{ $productSearch }}" data-product-status="{{ $productStatus }}" data-highlight-variant="{{ $adjustingVariantId ?: '' }}" hidden>
                                    ...
                                    <div id="product-variant-location-notice" class="inventory-variant-location-notice" role="status" aria-live="polite" hidden><i class="bi bi-geo-alt"></i><span>Selecciona una ubicación arriba para habilitar el saldo, costo de referencia y guardado de cada talla.</span></div>
                                    @if($canCreate)
...
<style>
    ...
    .inventory-kizeo-origin { display:flex; flex-wrap:wrap; gap:.45rem; margin:0 0 1rem; }.inventory-kizeo-origin-link { display:inline-flex; align-items:center; min-height:2.15rem; padding:.35rem .75rem; color:#4b397d; background:#fff; border:1px solid #d8cffa; border-radius:999px; font-size:.78rem; font-weight:750; text-decoration:none; }.inventory-kizeo-origin-link:hover { border-color:#a78bfa; background:#f7f4ff; }.inventory-kizeo-origin-link.active { color:#fff; background:#4d28a5; border-color:#4d28a5; }.dark-mode .inventory-kizeo-origin-link { color:#e7ddff; background:#1f1934; border-color:#5c478f; }.dark-mode .inventory-kizeo-origin-link.active { color:#fff; background:#6d28d9; border-color:#6d28d9; }
    .inventory-kizeo-period-panel { display:grid; grid-template-columns:minmax(210px,1fr) auto; gap:.8rem 1rem; align-items:end; margin:0 0 1rem; padding:.8rem 1rem; background:linear-gradient(105deg,#f1faf5,#f8fbff); border:1px solid #d8ebe0; border-radius:.7rem; }.inventory-kizeo-period-summary { display:flex; align-items:center; gap:.58rem; min-width:0; }.inventory-kizeo-sync-indicator { width:.62rem; height:.62rem; flex:0 0 auto; border-radius:50%; background:#22c55e; box-shadow:0 0 0 .2rem rgba(34,197,94,.12); }.inventory-kizeo-period-summary strong,.inventory-kizeo-period-summary span,.inventory-kizeo-period-summary small { display:block; }.inventory-kizeo-period-summary strong { color:#234638; font-size:.84rem; font-weight:800; }.inventory-kizeo-period-summary span { margin-top:.08rem; color:#587068; font-size:.76rem; }.inventory-kizeo-period-summary small { margin-top:.12rem; color:#718096; font-size:.7rem; }.inventory-kizeo-period-form { display:flex; align-items:end; justify-content:flex-end; flex-wrap:wrap; gap:.55rem; margin:0; }.inventory-kizeo-period-form label,.inventory-kizeo-custom-range label { display:grid; gap:.24rem; min-width:0; color:#53627d; font-size:.69rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; }.inventory-kizeo-period-form .form-select,.inventory-kizeo-period-form .form-control { min-height:2.35rem; margin:0; font-size:.82rem; letter-spacing:0; text-transform:none; }.inventory-kizeo-period-form > label { width:11rem; }.inventory-kizeo-custom-range { display:none; gap:.45rem; }.inventory-kizeo-custom-range.is-visible { display:grid; grid-template-columns:repeat(2,minmax(8rem,1fr)); }.inventory-kizeo-period-form .inventory-btn { min-height:2.35rem; }.inventory-kizeo-quick-periods { grid-column:1/-1; display:flex; flex-wrap:wrap; gap:.18rem; padding-top:.1rem; }.inventory-kizeo-quick-periods a { display:inline-flex; align-items:center; min-height:1.85rem; padding:.25rem .55rem; color:#5e6d86; border-radius:.38rem; font-size:.73rem; font-weight:750; text-decoration:none; }.inventory-kizeo-quick-periods a:hover { color:#2e1a68; background:#e9f3ec; }.inventory-kizeo-quick-periods a.active { color:#fff; background:#245c42; }.dark-mode .inventory-kizeo-period-panel { background:rgba(22,101,52,.15); border-color:#285442; }.dark-mode .inventory-kizeo-period-summary strong { color:#d1fae5; }.dark-mode .inventory-kizeo-period-summary span,.dark-mode .inventory-kizeo-period-summary small { color:#aeb9cc; }.dark-mode .inventory-kizeo-quick-periods a { color:#cbd5e1; }.dark-mode .inventory-kizeo-quick-periods a:hover { color:#ecfdf5; background:#1b4232; }.dark-mode .inventory-kizeo-quick-periods a.active { color:#fff; background:#26704f; }
    @container (max-width: 980px) { .inventory-kizeo-period-panel { grid-template-columns:1fr; align-items:stretch; }.inventory-kizeo-period-form { justify-content:flex-start; } }
    @container (max-width: 600px) { .inventory-kizeo-period-panel { padding:.75rem; }.inventory-kizeo-period-form { display:grid; grid-template-columns:1fr; }.inventory-kizeo-period-form > label { width:auto; }.inventory-kizeo-custom-range.is-visible { grid-template-columns:1fr; }.inventory-kizeo-period-form .inventory-btn { width:100%; }.inventory-kizeo-quick-periods { gap:.1rem; }.inventory-kizeo-quick-periods a { padding-inline:.42rem; } }
    .inventory-summary-period-panel { background:linear-gradient(105deg,#f4f8ff,#f8fbff); border-color:#d9e5f8; }.inventory-summary-period-panel .inventory-kizeo-sync-indicator { background:#2563eb; box-shadow:0 0 0 .2rem rgba(37,99,235,.12); }.inventory-summary-period-panel .inventory-kizeo-period-summary strong { color:#294d91; }.inventory-summary-period-panel .inventory-kizeo-period-summary span { color:#5c7096; }.inventory-summary-period-panel .inventory-kizeo-quick-periods a:hover { color:#1e3a74; background:#e8f0ff; }.inventory-summary-period-panel .inventory-kizeo-quick-periods a.active { background:#294d91; }.dark-mode .inventory-summary-period-panel { background:rgba(30,64,175,.16); border-color:#35599a; }.dark-mode .inventory-summary-period-panel .inventory-kizeo-period-summary strong { color:#dbeafe; }.dark-mode .inventory-summary-period-panel .inventory-kizeo-period-summary span { color:#bfdbfe; }.dark-mode .inventory-summary-period-panel .inventory-kizeo-quick-periods a:hover { color:#dbeafe; background:#1e3a74; }.dark-mode .inventory-summary-period-panel .inventory-kizeo-quick-periods a.active { background:#315caa; }
    .inventory-kizeo-intro { display:flex; align-items:center; justify-content:space-between; gap:1rem; padding:1rem 1.1rem; background:#f2f6ff; border:1px solid #dbe5fb; border-left:4px solid #2563eb; border-radius:.65rem; margin-bottom:1rem; }.inventory-kizeo-intro h2 { font-size:1.05rem; margin:0 0 .22rem; font-weight:800; }.inventory-kizeo-intro p:not(.inventory-kicker) { color:#61708b; font-size:.83rem; margin:0; max-width:720px; }.inventory-kizeo-intro-actions { display:flex; align-items:center; flex-wrap:wrap; justify-content:flex-end; gap:.65rem; }.inventory-kizeo-auto-form { margin:0; }.inventory-kizeo-switch { display:flex; align-items:center; gap:.7rem; min-height:3.1rem; padding:.45rem .7rem .45rem .5rem; color:#3d2a71; background:#fff; border:1px solid #d8cffa; border-radius:.7rem; text-align:left; cursor:pointer; }.inventory-kizeo-switch:hover { border-color:#a78bfa; background:#f7f4ff; }.inventory-kizeo-switch.is-on { color:#0f6b3c; background:#ecfdf3; border-color:#86efac; }.inventory-kizeo-switch strong,.inventory-kizeo-switch small { display:block; }.inventory-kizeo-switch strong { font-size:.8rem; font-weight:800; }.inventory-kizeo-switch small { margin-top:.12rem; color:#6d7a91; font-size:.72rem; line-height:1.3; font-weight:600; }.inventory-kizeo-switch.is-on small { color:#087245; }.inventory-kizeo-switch-track { position:relative; display:inline-flex; width:2.4rem; height:1.35rem; flex:0 0 auto; background:#d7dce6; border-radius:999px; }.inventory-kizeo-switch.is-on .inventory-kizeo-switch-track { background:#16a34a; }.inventory-kizeo-switch-thumb { position:absolute; top:.16rem; left:.16rem; width:1.02rem; height:1.02rem; background:#fff; border-radius:50%; transition:left .16s ease; }.inventory-kizeo-switch.is-on .inventory-kizeo-switch-thumb { left:1.2rem; }.dark-mode .inventory-kizeo-switch { color:#e7ddff; background:#1f1934; border-color:#5c478f; }.dark-mode .inventory-kizeo-switch.is-on { color:#bbf7d0; background:#052e16; border-color:#166534; }.dark-mode .inventory-kizeo-switch small { color:#aeb9cc; }.dark-mode .inventory-kizeo-switch.is-on small { color:#86efac; }.inventory-kizeo-kpis { grid-template-columns:repeat(4,minmax(0,1fr)); }.inventory-kizeo-notice { display:flex; align-items:flex-start; gap:.65rem; padding:.8rem 1rem; background:#fff8e8; border:1px solid #f2d392; border-radius:.6rem; color:#71501a; margin-bottom:1rem; font-size:.82rem; }.inventory-kizeo-notice i { color:#d17913; margin-top:.05rem; }.inventory-kizeo-notice strong,.inventory-kizeo-notice span { display:block; }.inventory-kizeo-notice span { margin-top:.12rem; line-height:1.45; }.inventory-kizeo-queue { display:grid; gap:.85rem; }.inventory-delivery-card { background:#fff; border:1px solid #dfe6f0; border-radius:.65rem; box-shadow:0 .35rem 1rem rgba(29,41,67,.04); overflow:hidden; }.inventory-delivery-card.needs-review { border-color:#f0bf74; }.inventory-delivery-header { display:flex; justify-content:space-between; align-items:flex-start; gap:1rem; padding:.9rem 1rem; border-bottom:1px solid #e7ecf3; }.inventory-delivery-header h3 { margin:.25rem 0 .12rem; font-size:.95rem; font-weight:800; }.inventory-delivery-header p { color:#708099; font-size:.78rem; margin:0; }.inventory-delivery-status { display:flex; align-items:center; justify-content:flex-end; flex-wrap:wrap; gap:.55rem; }.inventory-kizeo-form { display:grid; gap:.75rem; padding:.9rem 1rem 1rem; }.inventory-kizeo-form-heading { display:flex; align-items:end; justify-content:space-between; gap:1rem; }.inventory-kizeo-form-heading label { display:grid; gap:.28rem; min-width:280px; color:#53627d; font-size:.72rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; }.inventory-kizeo-form-heading .form-select { min-height:2.42rem; font-size:.88rem; letter-spacing:0; text-transform:none; }.inventory-kizeo-form-heading p { color:#738097; font-size:.79rem; margin:0; max-width:460px; line-height:1.4; }.inventory-delivery-summary { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:1px; background:#e4eaf2; }.inventory-delivery-summary > div { padding:.7rem 1rem; background:#fbfcfe; }.inventory-delivery-summary span { display:block; color:#748198; font-size:.68rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; }.inventory-delivery-summary strong { display:block; color:#273450; font-size:.82rem; margin-top:.18rem; }.inventory-source-warning { display:flex; gap:.55rem; margin:.8rem 1rem 0; padding:.7rem .8rem; background:#fff4e5; border-left:3px solid #e88120; color:#875115; font-size:.8rem; line-height:1.4; }.inventory-reverse-details { margin:.75rem 1rem 1rem; background:#f8fafc; border:1px solid #e2e8f0; border-radius:.5rem; }.inventory-reverse-details summary { cursor:pointer; display:flex; align-items:center; gap:.48rem; padding:.65rem .75rem; color:#4f3b78; font-size:.8rem; font-weight:800; }.inventory-reverse-form { display:grid; grid-template-columns:minmax(0,1fr) auto; align-items:end; gap:.65rem; padding:.1rem .75rem .75rem; }.inventory-reverse-form label { display:grid; gap:.28rem; color:#53627d; font-size:.72rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; }.inventory-reverse-form .form-control { min-height:2.42rem; font-size:.88rem; letter-spacing:0; text-transform:none; }
    .inventory-kizeo-stock { display:inline-flex; flex-direction:column; align-items:flex-end; min-width:5.75rem; padding:.32rem .5rem; border:1px solid #dce3ed; border-radius:.42rem; background:#f8fafc; line-height:1.15; white-space:nowrap; }.inventory-kizeo-stock span { color:#6d7a91; font-size:.65rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; }.inventory-kizeo-stock strong { margin-top:.12rem; color:#34415b; font-size:.84rem; }.inventory-kizeo-stock.is-ok { background:#ecfdf3; border-color:#b7ebca; }.inventory-kizeo-stock.is-ok span,.inventory-kizeo-stock.is-ok strong { color:#087245; }.inventory-kizeo-stock.is-low { background:#fff0f1; border-color:#f4b8bd; }.inventory-kizeo-stock.is-low span,.inventory-kizeo-stock.is-low strong { color:#b42332; }.inventory-kizeo-stock.is-unlinked { background:#f5f7fa; border-color:#dbe1ea; }.dark-mode .inventory-kizeo-stock { background:rgba(148,163,184,.1); border-color:#475569; }.dark-mode .inventory-kizeo-stock span,.dark-mode .inventory-kizeo-stock strong { color:#cbd5e1; }.dark-mode .inventory-kizeo-stock.is-ok { background:rgba(16,185,129,.13); border-color:rgba(52,211,153,.35); }.dark-mode .inventory-kizeo-stock.is-ok span,.dark-mode .inventory-kizeo-stock.is-ok strong { color:#86efac; }.dark-mode .inventory-kizeo-stock.is-low { background:rgba(244,63,94,.13); border-color:rgba(251,113,133,.35); }.dark-mode .inventory-kizeo-stock.is-low span,.dark-mode .inventory-kizeo-stock.is-low strong { color:#fda4af; }
    @container (max-width: 760px) { .inventory-kizeo-form .inventory-table tbody td:nth-child(5) { grid-column:1/-1; margin-top:.18rem; }.inventory-kizeo-form .inventory-table tbody td:nth-child(5)::before { content:'Stock SAEP · Sede Central'; display:block; margin-bottom:.28rem; color:#53627d; font-size:.68rem; font-weight:800; letter-spacing:.035em; text-align:left; text-transform:uppercase; }.inventory-kizeo-form .inventory-table tbody td:nth-child(5) .inventory-kizeo-stock { align-items:flex-start; }.dark-mode .inventory-kizeo-form .inventory-table tbody td:nth-child(5)::before { color:#aeb9cc; } }
    .inventory-kizeo-adjust-link { display:inline-flex; align-items:center; gap:.3rem; margin-top:.38rem; color:#351178; font-size:.72rem; font-weight:800; line-height:1.2; text-decoration:none; white-space:nowrap; }.inventory-kizeo-adjust-link:hover { color:#23085d; text-decoration:underline; }.inventory-kizeo-adjust-link:focus-visible { outline:2px solid rgba(114,80,202,.32); outline-offset:2px; border-radius:.2rem; }.dark-mode .inventory-kizeo-adjust-link { color:#c4b5fd; }
    ...
    .inventory-kizeo-application-lines { display:grid; gap:.75rem; padding:.8rem 1rem 0; }.inventory-kizeo-line-panel { overflow:hidden; border:1px solid #dce5f1; border-radius:.55rem; background:#fbfcff; }.inventory-kizeo-line-panel.is-current-source { border-color:#f3cf9e; background:#fffaf2; }.inventory-kizeo-line-panel-heading { display:flex; align-items:flex-start; justify-content:space-between; gap:.8rem; padding:.7rem .8rem; border-bottom:1px solid #e4eaf2; }.inventory-kizeo-line-panel-heading strong,.inventory-kizeo-line-panel-heading span { display:block; }.inventory-kizeo-line-panel-heading strong { color:#2a3b59; font-size:.81rem; font-weight:850; }.inventory-kizeo-line-panel-heading strong i { margin-right:.3rem; color:#5b3ca2; }.inventory-kizeo-line-panel.is-current-source .inventory-kizeo-line-panel-heading strong i { color:#c26a12; }.inventory-kizeo-line-panel-heading span { max-width:46rem; margin-top:.15rem; color:#6f7e95; font-size:.74rem; line-height:1.35; }.inventory-kizeo-line-panel-heading small { flex:0 0 auto; padding:.22rem .45rem; color:#52627d; background:#eef2f8; border-radius:999px; font-size:.68rem; font-weight:750; white-space:nowrap; }.inventory-kizeo-line-panel.is-current-source .inventory-kizeo-line-panel-heading small { color:#875115; background:#fff0d9; }.inventory-kizeo-application-lines .inventory-table { min-width:40rem; }.inventory-kizeo-application-lines .inventory-table td small { display:block; margin-top:.12rem; color:#738097; font-size:.69rem; }.inventory-kizeo-discounted-quantity { color:#b42332; }.inventory-muted { color:#738097; font-size:.78rem; font-style:italic; }.dark-mode .inventory-kizeo-line-panel { background:rgba(255,255,255,.035); border-color:#475569; }.dark-mode .inventory-kizeo-line-panel.is-current-source { background:rgba(180,83,9,.12); border-color:#925f24; }.dark-mode .inventory-kizeo-line-panel-heading { border-color:#475569; }.dark-mode .inventory-kizeo-line-panel-heading strong { color:#e5edf9; }.dark-mode .inventory-kizeo-line-panel-heading span,.dark-mode .inventory-kizeo-application-lines .inventory-table td small,.dark-mode .inventory-muted { color:#aeb9cc; }.dark-mode .inventory-kizeo-line-panel-heading small { color:#cbd5e1; background:#334155; }.dark-mode .inventory-kizeo-line-panel.is-current-source .inventory-kizeo-line-panel-heading small { color:#fed7aa; background:#542a0b; }.dark-mode .inventory-kizeo-discounted-quantity { color:#fda4af; }
    .inventory-kizeo-line-panel.is-correction { border-color:#a7e3c1; background:#f3fcf7; }.inventory-kizeo-line-panel.is-correction .inventory-kizeo-line-panel-heading strong i { color:#15803d; }.inventory-kizeo-line-panel.is-correction .inventory-kizeo-line-panel-heading small { color:#087245; background:#dff7e8; }.inventory-kizeo-restocked-quantity { color:#087245; }.inventory-kizeo-correction-action { display:inline-flex; align-items:center; gap:.3rem; padding:.24rem .45rem; border-radius:999px; font-size:.72rem; font-weight:800; white-space:nowrap; }.inventory-kizeo-correction-action.is-restocked { color:#087245; background:#dcfce7; }.inventory-kizeo-correction-action.is-discounted { color:#b42332; background:#fff0f1; }.inventory-kizeo-correction-detail { color:#53627d; font-size:.75rem; line-height:1.35; }.dark-mode .inventory-kizeo-line-panel.is-correction { background:rgba(22,163,74,.12); border-color:#267a4a; }.dark-mode .inventory-kizeo-line-panel.is-correction .inventory-kizeo-line-panel-heading small { color:#bbf7d0; background:#14532d; }.dark-mode .inventory-kizeo-restocked-quantity { color:#86efac; }.dark-mode .inventory-kizeo-correction-action.is-restocked { color:#bbf7d0; background:rgba(22,163,74,.24); }.dark-mode .inventory-kizeo-correction-action.is-discounted { color:#fecdd3; background:rgba(225,29,72,.22); }.dark-mode .inventory-kizeo-correction-detail { color:#aeb9cc; }
    @container (max-width: 620px) { .inventory-kizeo-line-panel-heading { align-items:flex-start; flex-direction:column; }.inventory-kizeo-application-lines .inventory-table { min-width:32rem; } }
    ...
    .inventory-product-search { display:grid; grid-template-columns:minmax(220px,1fr) minmax(150px,180px) auto auto; align-items:end; justify-content:start; gap:.6rem; margin:1rem 0 .8rem; }
    .inventory-product-search label { display:grid; gap:.28rem; min-width:0; color:#53627d; font-size:.72rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; }
    .inventory-product-search .form-control, .inventory-product-search .form-select { max-width:none; margin:0; min-height:2.42rem; }
    ...
    @container (max-width: 700px) {
        ...
        .inventory-product-search { grid-template-columns:minmax(0,1fr) minmax(140px,180px) auto auto; }
    ...
    @container (max-width: 440px) {
        ...
        .inventory-product-search { grid-template-columns:1fr 2.55rem; }
        .inventory-product-search label { grid-column:1 / -1; }
        .inventory-product-search .inventory-btn { grid-column:1; width:100%; }
        .inventory-product-search .inventory-icon-btn { grid-column:2; }
    }
    ...
    .inventory-variant-location-notice { display:flex; gap:.55rem; align-items:center; padding:.7rem .8rem; border:1px solid #f0c56b; border-left:4px solid #e99a10; border-radius:.5rem; background:#fff9ec; color:#714d08; font-size:.8rem; font-weight:700; line-height:1.4; }.inventory-variant-location-notice i { font-size:1rem; }.dark-mode .inventory-variant-location-notice { border-color:#9f7021; background:rgba(163,110,17,.16); color:#f2d79a; }
    ...
    .inventory-history-filter { display:grid; grid-template-columns:minmax(17rem,2fr) repeat(4,minmax(8.5rem,1fr)) auto; gap:.7rem; align-items:end; margin:.85rem 0 .65rem; padding:.85rem; background:#f8fafc; border:1px solid #e2e8f0; border-radius:.6rem; }.inventory-history-filter label { display:grid; gap:.28rem; min-width:0; color:#53627d; font-size:.7rem; font-weight:800; letter-spacing:.035em; text-transform:uppercase; }.inventory-history-filter .form-control,.inventory-history-filter .form-select { min-height:2.4rem; margin:0; font-size:.84rem; letter-spacing:0; text-transform:none; }.inventory-history-filter-actions { display:flex; align-items:end; gap:.42rem; min-height:2.4rem; }.inventory-history-filter-actions .inventory-btn { min-height:2.4rem; white-space:nowrap; }.inventory-history-summary { margin:0 0 .65rem; color:#667895; font-size:.77rem; font-weight:700; }.dark-mode .inventory-history-filter { background:rgba(255,255,255,.035); border-color:#374151; }.dark-mode .inventory-history-filter label,.dark-mode .inventory-history-summary { color:#aeb9cc; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var receiptType = document.querySelector('[data-receipt-type]');
    var receiptCostCenterField = document.querySelector('[data-receipt-cost-center]');
    var receiptCostCenterUnavailable = document.querySelector('[data-receipt-cost-center-unavailable]');
    var receiptCostCenterSelect = receiptCostCenterField ? receiptCostCenterField.querySelector('select') : null;
    var receiptContext = document.querySelector('[data-receipt-context]');
    var receiptObservation = document.querySelector('[data-receipt-observation]');
    function toggleReceiptType() {
        if (!receiptType) return;
        var isReturn = receiptType.value === 'DEVOLUCION_EPP';
        if (receiptCostCenterField) receiptCostCenterField.hidden = !isReturn;
        if (receiptCostCenterUnavailable) receiptCostCenterUnavailable.hidden = !isReturn;
        if (receiptCostCenterSelect) receiptCostCenterSelect.required = isReturn;
        if (receiptContext) {
            receiptContext.textContent = isReturn
                ? 'Esta devolución aumenta el stock de Bodega y se imputa al centro de costo seleccionado.'
                : 'Estás registrando una compra o reposición de Bodega.';
        }
        if (receiptObservation) {
            receiptObservation.placeholder = isReturn
                ? 'Motivo o referencia de la devolución'
                : 'Compra, reposición u otra referencia';
        }
    }
    if (receiptType) { receiptType.addEventListener('change', toggleReceiptType); toggleReceiptType(); }
    ...
    var productVariantLocationNotice = document.getElementById('product-variant-location-notice');
    ...
    function renderProductVariantCards() {
        ...
        if (productVariantLocationNotice) productVariantLocationNotice.hidden = Boolean(locationId);
        productVariantRows.replaceChildren();
        ...
        variants.forEach(function (variant) {
            ...
            var disabledReason = !variant.activo
                ? 'Reactiva esta talla para editarla.'
                : (!locationId ? 'Selecciona una ubicación arriba para habilitar la edición.' : '');
            if (disabledReason) button.title = disabledReason;
            button.append(
                inventoryElement('i', locationId && variant.activo ? 'bi bi-save2' : 'bi bi-geo-alt'),
                document.createTextNode(locationId && variant.activo ? 'Guardar ' + titleText : (variant.activo ? 'Selecciona una ubicación arriba' : 'Talla inactiva'))
            );
    ...
    var kizeoPeriodSelect = document.querySelector('[data-kizeo-period-select]');
    var kizeoCustomRange = document.querySelector('[data-kizeo-custom-range]');
    function refreshKizeoCustomRange() {
        if (!kizeoPeriodSelect || !kizeoCustomRange) return;
        var isCustom = kizeoPeriodSelect.value === 'personalizado';
        kizeoCustomRange.classList.toggle('is-visible', isCustom);
        kizeoCustomRange.querySelectorAll('input').forEach(function (input) { input.disabled = !isCustom; });
    }
    if (kizeoPeriodSelect) {
        kizeoPeriodSelect.addEventListener('change', refreshKizeoCustomRange);
        refreshKizeoCustomRange();
    }

    var summaryPeriodSelect = document.querySelector('[data-summary-period-select]');
    var summaryCustomRange = document.querySelector('[data-summary-custom-range]');
    function refreshSummaryCustomRange() {
        if (!summaryPeriodSelect || !summaryCustomRange) return;
        var isCustom = summaryPeriodSelect.value === 'personalizado';
        summaryCustomRange.classList.toggle('is-visible', isCustom);
        summaryCustomRange.querySelectorAll('input').forEach(function (input) { input.disabled = !isCustom; });
    }
    if (summaryPeriodSelect) {
        summaryPeriodSelect.addEventListener('change', refreshSummaryCustomRange);
        refreshSummaryCustomRange();
    }
    ...
    var kizeoStockData = document.getElementById('inventory-kizeo-central-stock');
    var kizeoCentralStockByVariant = {};
    if (kizeoStockData) {
        try {
            kizeoCentralStockByVariant = JSON.parse(kizeoStockData.textContent || '{}');
        } catch (error) {
            kizeoCentralStockByVariant = {};
        }
    }
    function formatKizeoStock(quantity) {
        return new Intl.NumberFormat('es-CL', { maximumFractionDigits: 3 }).format(quantity);
    }
    function refreshKizeoLineStock(select) {
        var row = select.closest('tr');
        var output = row ? row.querySelector('[data-kizeo-stock]') : null;
        if (!output) return;

        var variantId = select.value;
        var requested = Number(output.dataset.requestedQuantity || 0);
        var available = variantId && Object.prototype.hasOwnProperty.call(kizeoCentralStockByVariant, variantId)
            ? Number(kizeoCentralStockByVariant[variantId])
            : 0;
        var hasRelation = Boolean(variantId);
        var isLow = hasRelation && available < requested;

        output.classList.toggle('is-unlinked', !hasRelation);
        output.classList.toggle('is-low', isLow);
        output.classList.toggle('is-ok', hasRelation && !isLow);
        output.replaceChildren();

        var label = document.createElement('span');
        var value = document.createElement('strong');
        label.textContent = !hasRelation ? 'Sin relación' : (isLow ? 'Insuficiente' : 'Disponible');
        value.textContent = hasRelation ? formatKizeoStock(available) : '—';
        output.append(label, value);
    }
    document.querySelectorAll('[data-kizeo-stock-select]').forEach(function (select) {
        select.addEventListener('change', function () { refreshKizeoLineStock(select); });
        refreshKizeoLineStock(select);
    });
