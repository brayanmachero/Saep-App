...
class ComercialCotizacionFlowTest extends TestCase
{
    ...
    public function test_calcula_modalidades_est_y_sub_con_totales_coherentes(): void
    {
        $this->createCommercialFixture();

        $calculadora = app(CalculadoraCotizacionService::class);

        foreach (['EST', 'SUB'] as $codigoModalidad) {
            $modalidad = Modalidad::where('codigo', $codigoModalidad)->firstOrFail();
            $resultado = $calculadora->calcular($this->calculationPayload($modalidad, 720000));

            $this->assertSame($codigoModalidad, $resultado['modalidad']);
            $this->assertGreaterThan(0, $resultado['total_remuneraciones']);
            $this->assertGreaterThan($resultado['subtotal'], $resultado['precio_venta']);
            $this->assertGreaterThan(0, $resultado['horas']['normal']);
            $this->assertContains('Gratificación Legal', collect($resultado['detalles'])->pluck('concepto')->all());
        }
    }
    ...
    public function test_flujo_crea_aprueba_reemplaza_duplica_y_marca_no_vigente(): void
    {
        ...
        $this->assertGreaterThan(0, (float) $primera->precio_venta);
        ...
        $this->assertNotNull($primera->fecha_vigencia_hasta);
