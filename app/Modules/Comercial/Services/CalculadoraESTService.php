...
class CalculadoraESTService
{
    public function calcular(array $datos): array
    {
        ...
        $refprevPct = (float) Parametro::valor('REFPREV_EST', 1);
        $sisPct = (float) Parametro::valor('SIS_EST', 1.49);
        $mutualPct = (float) Parametro::valor('MUTUAL_EST', 1.27);
        $cesantiaPct = (float) Parametro::valor('CESANTIA_EST', 3);
        $vacacionesDias = (float) Parametro::valor('VACACIONES_DIAS_EST', 21);
        $gastosAdminPct = (float) Parametro::valor('GASTOS_ADMIN_EST', 3);
        $margenPct = (float) Parametro::valor('MARGEN_EST', 10);
        $horasMensuales = (float) Parametro::valor('HORAS_MENSUALES_EST', 180);
        $horasHhee = (float) Parametro::valor('HORAS_HHEE_EST', 176);
        ...
        $provisionVacaciones = ((($totalImponible + $sis + $mutual + $seguroCesantia + $refprev) / 12) / 30) * $vacacionesDias;
        ...
        $totalProvisiones = $provisionVacaciones;
        ...
        $horaNormalHhee = $horasHhee > 0 ? $precioVentaHhee / $horasHhee : 0;
    ...
    private function normalizarEntrada(array $datos): array
    {
        $base = [
            ...
            'otros_beneficios' => (float) ($datos['otros_beneficios'] ?? Parametro::valor('AGUINALDO_EST', 5000)),
        ];
    ...
    private function construirDetalles(array $i, array $r, array $tasas): array
    {
        return [
            ...
            $this->detalle('provision', 'Provisión Vacaciones', $r['totalImponible'], null, $r['provisionVacaciones'], 200, '(((Total imponible + ISES) / 12) / 30) * días vacaciones EST'),
