...
class OneDriveService
{
    ...
    private function uploadToSiteId(string $token, string $siteId, string $fullPath, string $content, string $contentType, string $context): bool
    {
        ...
        if ($response->successful()) {
            ...
            $this->lastUploadResult['ok'] = true;
            $this->lastUploadResult['item_id'] = $response->json('id');
    ...
    private function recordUploadError(string $message, array $context = []): void
    {
        ...
        $this->lastUploadResult['ok'] = false;
        $this->lastUploadResult['status'] = $context['status'] ?? ($this->lastUploadResult['status'] ?? null);
        $this->lastUploadResult['path'] = $context['path'] ?? ($this->lastUploadResult['path'] ?? null);
        $this->lastUploadResult['error'] = $message
            . (isset($context['status']) ? ': HTTP ' . $context['status'] : '')
            . (isset($context['body']) ? ' ' . substr($context['body'], 0, 500) : '');
